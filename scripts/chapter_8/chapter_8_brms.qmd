...
dagify(R ~ U,
  ...
  geom_point(x = 2, y = 1, size = 6, shape = 1, stroke = 3/4,
...
```{r}
# make the log version of the criterion
d <-
  d %>% 
...
p1 <-
  ...
  geom_text_repel(data = . %>% 
                    dplyr::filter(country %in% c("Lesotho", "Seychelles")),
                  aes(label = country),
...
p2 <-
  ...
  geom_text_repel(data = . %>% 
                    dplyr::filter(country %in% c("Switzerland", "Tajikistan")),
                  aes(label = country),
                  size = 3, family = "Times", seed = 8) +
  xlim(0, 1) +
...
p1 <-
  ...
  labs(subtitle = "Intercept ~ dnorm(1, 1)\nb ~ dnorm(0, 1)",
       x = "ruggedness", y = "log GDP (prop of mean)") +
...
loo_compare(b8.1b, b8.2, criterion = "waic") %>% 
  print(simplify = F)
```

```{r}
model_weights(b8.1b, b8.2, weights = "waic") %>% 
  round(digits = 3)
```
...
dd %>% 
  ...
  ggplot(aes(x = rugged_std, fill = cont_africa,
             color = cont_africa)) +
  geom_smooth(data = f,
  ...
  scale_fill_pander() +
  scale_color_pander() +
  labs(subtitle = "b8.2", x = "ruggedness (standardized)",
       y = "log GDP (as proportion of mean)") +
  coord_cartesian(xlim = c(0, 1)) +
...
```{r}
model_weights(b8.1b, b8.2, b8.3, weights = "loo") %>% 
  round(digits = 2)
```
...
f <-
  nd %>% 
  add_epred_draws(b8.3) %>% 
  mean_qi(.epred, .width = .97) %>% 
  mutate(cont_africa = ifelse(cid == 1, "African nations",
                              "Non-African nations"))
...
dd %>% 
  ...
  geom_smooth(data = f,
              ...
              linewidth = 1/2) +
  geom_text_repel(data = . %>% dplyr::filter(country %in% countries),
                  aes(label = country), size = 3, seed = 8,
...
tibble(a = rnorm(1e4, mean = .5, sd = .25)) %>% 
  summarise(proportion_outside_of_the_range = sum(a < 0 | a > 1) / n())
```
...
b8.4 <-
  brm(data = d,
      ...
      prior = c(prior(normal(0.5, .25), class = Intercept),
                prior(normal(0, .25), class = b, coef = water_cent),
                prior(normal(0, .25), class = b, coef = shade_cent),
                prior(exponential(1), class = sigma)),
      iter = 2000, warmup = 1000, chains = 4, cores = 4,
      seed = 8,
      file = "fits/b08.04")
...
b8.5 <-
  brm(data = d,
      ...
      prior = c(prior(normal(.5, .25), class = Intercept),
                prior(normal(0, .25), class = b, coef = water_cent),
                prior(normal(0, .25), class = b, coef = shade_cent),
                prior(normal(0, .25), class = b, coef = "water_cent:shade_cent"),
                prior(exponential(1), class = sigma)),
      iter = 2000, warmup = 1000, chains = 4, cores = 4,
      seed = 8,
      file = "fits/b08.05")
...
for(s in -1:1){
  ...
  fig <-
    ...
    geom_line(data = f,
              aes(group = row),
              color = palette_pander(n = 6)[6], alpha = 1/5,
              linewidth = 1/2) +
    geom_point(color = palette_pander(n = 6)[6]) +
    scale_x_continuous("Water (centered)", 
                       breaks = c(-1, 0, 1)) +
...
We don't need a loop we can use facet_grid
```{r}
# augment the data
points <-
  d %>% 
...
rbind(fitted(b8.4, newdata = nd, summary = F, ndraws = 20),
  ...
  set_names(
    mutate(nd, 
           name = str_c(shade_cent, water_cent, sep = "_")) %>% 
      pull()) %>% 
  mutate(row = 1:n(),
...
```{r}
b8.3$data %>% 
  summarise(mean = mean(rugged_std_c),
