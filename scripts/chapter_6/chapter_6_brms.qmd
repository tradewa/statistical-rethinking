...
d %>% 
  ggplot(aes(x = newsworthiness, y = trustworthiness, colour = selected)) +
...
Now use sim_and_fit() to make our simulations which correspond to seed values 1:4.
```{r}
sim <-
  tibble(seed = 1:4) %>% 
...
Now the betas are smaller and less certain
```{r}
posterior_summary(b6.5)[1:3, ] %>% round(digits = 2)
```

Here's a quick pairs plot
```{r}
d %>% 
...
#### 6.1.2.2 Overthinking: Simulating colllinearity
```{r}
...
d %>% 
  ...
  geom_smooth(aes(ymin = ll, ymax = ul),
              ...
              color = "orange", alpha = 1/3, linewidth = 2/3) +
  labs(x = expression(rho), y = "parameter SD") +
...
d2 <-
  tibble(h0 = rnorm(n, mean = 10, sd = 2),
         ...
         fungus = rbinom(n, size = 1, 
                         prob = .5 - treatment * 0.4 + 0.4 * m),
...
new_borns <- function(n = 20){
  tibble(a = 1, # 1 year old
         m = 0, # not married
         h = seq(from = -2, to = 2, length.out = n)) # range of happiness score
}
```
...
```{r}
update_population <- function(pop, n_births = 20, aom = 18,
                              max_age = 65){
  pop %>% 
...
```{r}
new_borns() %>% update_population()
```

Run the population for 1,000 years
```{r}
...
d %>% 
  mutate(married = 
           factor(married, labels = c("unmarried", "married"))) %>% 
...
d <-
  ...
  mutate(c = rnorm(n, mean = b_pc * p + b_gc * g
                   + b_u * u, sd = 1))
...
```{r}
d %>% 
  mutate(centile = ifelse(p >= quantile(p, prob = .45)
                          & p <= quantile(p, prob = .60),
                          "a", "b"),
         u = factor(u)) %>% 
...
d <-
  ...
  mutate(a = rethinking::standardize(MedianAgeMarriage),
         ...
         s = factor(South, levels = 0:1, labels = 
                      c("North", "South")),
