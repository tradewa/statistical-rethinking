...
$$
p_{admit} = \frac{\lambda_{1}}{\lambda_1 + \lambda_2} = \frac{\text{exp}(\alpha_1)}{\text{exp}(\alpha_1) + \text{exp}(\alpha_2)}
$$

To get a better appreciation on how well the two model types converge on the same solution, we might plot the full poster for admission probability from each.
```{r}
# wrangle
bind_cols(
...
d %>% 
  ...
  scale_x_continuous(expression(italic(n)), 
                     breaks = c(0, count(d, black) %>% 
                                  pull(n))) +
  labs(title = "Cat color", y = NULL) +
  theme(axis.ticks.y = element_blank())
```
...
Since we modeled log $\mu_i$, we need to transform our $\alpha$ paramaters back into the $\lambda$ metric using the formula
$$
\begin{align}
\text{log}\mu & = \alpha_{\text{black}}, && \text{and} \\
\lambda & = \frac{1}{\mu}, && \text{therefore} \\
\lambda_{\text{black}} & = \frac{1}{\text{exp}(\alpha_{\text{black}})}.
\end{align}
$$
...
post %>% 
  ...
  stat_halfeye(.width = .95, 
               fill = wes_palette("Moonrise2")[2],
               height = 4) +
  scale_x_continuous("days until 50% are adopted",
                     breaks = c(30, medians, 45),
                     labels = c("30", medians, "45"),
                     limits = c(30, 45)) +
  ylab(NULL) +
  coord_cartesian(ylim = c(1.5, 5.1)) +
  theme(axis.ticks.y = element_blank())
