...
d %>% 
  ...
  geom_text_repel(data = d %>% 
                    dplyr::filter(Loc %in% c("ME", "OK", "AR",
                                      "AL", "GA", "SC",
                                      "NJ")),
                  aes(label = Loc),
                  size = 4, seed = 1042) +
...
To get the posterior predictions from our brms model, we'll use fitted() in place of link(). But I want to use add_epred_draws instead
```{r}
# determine the range of 'a' values we'd like to feed into fitted()
nd <- tibble(a = seq(from = -3, to = 3.2, length.out = 30))

# now use 'fitted' to get the model-implied trajectories
add_epred_draws(nd, b5.1) %>% 
  mean_qi(.epred) %>% 
  ggplot(aes(x = a)) +
  geom_smooth(aes(y = .epred, ymin = .lower, ymax = .upper),
              stat = "identity", fill = "firebrick",
              color = "firebrick4", alpha = 1/5,
              linewidth = 1/4) +
  geom_point(data = d, aes(y = d), size = 2,
             color = "firebrick4") +
  labs(x = "Median age marriage (std)",
       y = "Divorce rate (std)") +
...
```{r}
nd <- tibble(m = seq(from = -3, to = 3, length.out = 30))

add_epred_draws(nd, b5.2) %>% 
  mean_qi(.epred) %>% 
  ggplot(aes(x = m)) +
  geom_smooth(aes(y = .epred, ymin = .lower, ymax = .upper),
              stat = "identity", color = "firebrick4",
...
b5.3 <-
  brm(data = d,
      ...
      prior = c(prior(normal(0, .2), class = Intercept),
                ...
                prior(normal(0, .5), class = b, coef = a),
                prior(exponential(1), class = sigma)),
      iter = 2000, warmup = 1000, chains = 4, cores = 4,
      seed = 5,
      file = "fits/b05.03")
...
sim_d <-
  ...
  mutate(mar = rnorm(n, mean = -age, sd = 1), # sim A -> M
         div = rnorm(n, mean = age, sd = 1)) # sim A _> D
...
If we use the update() function, we can refit the last model in haste
```{r}
b5.1_sim <-
  update(b5.1,
         newdata = sim_d,
...
```{r}
f <-
  add_epred_draws(newdata = b5.4$data, object = b5.4) %>% 
  mean_qi(.epred)
...
p1 <-
  ...
  geom_segment(aes(xend = a, yend = .epred),
               linewidth = 1/4) +
  geom_line(aes(y = .epred),
            color = "firebrick4") +
  geom_text_repel(data = d %>% dplyr::filter(Loc %in% c("WY", "ND", "ME", "HI", "DC")),  
                  aes(label = Loc), 
                  size = 3, seed = 14) +
...
p3 <-
  ...
  geom_text_repel(
    data = . %>% 
      dplyr::filter(Loc %in% c("WY", "ND", "ME", "HI", "DC")),
    aes(label = Loc), size = 3, seed = 5) +
...
```{r}
p2 <-
  add_epred_draws(newdata = b5.4b$data, object = b5.4b) %>% 
  mean_qi(.epred) %>% 

ggplot(aes(x = m, y = a)) +
  geom_line(aes(y = .epred),
            color = "firebrick4") +
  geom_segment(aes(xend = m, yend = .epred),
               linewidth = 1/4) +
  geom_point(data = d, 
             aes(y = a),
             shape = 1, size = 2, color = "firebrick4") +
  geom_text_repel(data = d %>% filter(Loc %in% c("DC", "HI", "ID")),  
                  aes(label = Loc), 
                  size = 3, seed = 5) +
  theme_bw() +
  labs(x = "Marriage rate (std)",
...
```{r}
p1 + p2 + p3 + p4 + 
  plot_annotation(title = "Understanding multiple regression through residuals")
```
...
```{r}
divorce_sd = sd(d$Divorce)
divorce_mean = mean(d$Divorce)

add_epred_draws(b5.3$data, b5.3) %>%
  mean_qi(.epred) %>%
  select(d, m, a, .epred, .lower, .upper) %>% 
  mutate_all(~ . * divorce_sd + divorce_mean) %>% 
  
  ggplot(aes(x = d, y = .epred)) +
...
set.seed(5)
d_spur <-
  tibble(x_real = rnorm(n), # x_real as Gaussian with mean 0 and SD 1
         x_spur = rnorm(n, x_real), # x_spur as Gaussian with mean = x_real
         y = rnorm(n, x_real)) # y as Gaussian with mean = x_real
```

```{r}
pairs(d_spur, col = "firebrick4oke o")
```

We may as well fit and evaluate the model
```{r}
b5.0_spur <-
  brm(data = d_spur,
      family = gaussian,
...
Specify each model separately in bf() function and save them as objects
```{r}
d_model <- bf(d ~ 1 + a + m)
m_model <- bf(m ~ 1 + a)
```
...
b5.3_A <-
  brm(data = d,
      ...
      prior = c(prior(normal(0, .2), class = Intercept, resp = d),
...
Use predict to make counterfactual plot in the left panel of Figure 5.6.
```{r}
nd <- tibble(a = seq(from = -2, to = 2, length.out = 30),
             m = 0)

p1 <-
  add_predicted_draws(nd, b5.3_A, resp = "d") %>% 
  mean_qi(.prediction) %>% 
  
  ggplot(aes(x = a, y = .prediction, ymin = .lower,
             ymax = .upper)) +
  geom_smooth(stat = "identity", fill = "firebrick",
              color = "firebrick4", alpha = 1/5,
              linewidth = 1/4) +
...
p2 <-
  add_predicted_draws(nd, b5.3_A, resp = "m") %>% 
  mean_qi(.prediction) %>% 
...
With our brms + tiduverse paradigm, we might compute "the expected causal effect of increasing median age at marriage from 20 to 30"
```{r}
# new data frame, standardized to mean 26.1 and std dev 1.24
nd <- tibble(name = c("a20", "a30"),
             a = (c(20, 30) - 26.1) / 1.24,
             m = 0)
...
post <-
  ...
  mutate(m_sim = rnorm(n(), mean = b_m_Intercept + b_m_a * a,
                       sd = sigma_m)) %>% 
  mutate(d_sim = rnorm(n(), mean = b_d_Intercept + b_d_a * a +
                         b_d_m * m_sim, sd = sigma_d)) %>% 
...
post %>% 
  ...
  labs(title = "Hand-made counterfactual plots for the multivariate divorce model",
       x = "manipulated A", y = "counterfactual") +
  coord_cartesian(ylim = c(-2, 2)) +
  theme_bw() + 
  theme(panel.grid = element_blank(),
        strip.background = element_blank()) +
...
prior_draws(b5.5_draft) %>% 
  ...
  geom_line(aes(group = rowname), color = "firebrick",
            alpha = .4) +
  coord_cartesian(ylim = c(-2, 2)) +
...
print(b5.5_draft)
```
Let's tighten up priors and fit b5.5
```{r}
b5.5 <-
  brm(data = dcc,
      family = gaussian,
...
nd %>% 
  add_epred_draws(b5.5) %>% 
  mean_qi(.epred, .width = c(.5, .95)) %>% 
  
  ggplot(aes(x = neocortex.perc_s, y = .epred)) +
  geom_ribbon(data = . %>% filter(.width == 0.50),
              aes(ymin = .lower, ymax = .upper),
              fill = "firebrick", alpha = 1/5) +
  geom_smooth(data = . %>% filter(.width == 0.95),
              aes(ymin = .lower, ymax = .upper),
              stat = "identity", fill = "firebrick4",
              color = "firebrick4", alpha = 1/5,
              linewidth = 1/2) +
  geom_point(data = dcc,
...
nd %>% 
  add_epred_draws(b5.6) %>% 
  mean_qi(.epred, .width = c(.5, .95)) %>% 
  
  ggplot(aes(x = log_mass_s, y = .epred)) +
  geom_ribbon(data = . %>% filter(.width == 0.5),
              aes(ymin = .lower, ymax = .upper),
              fill = "firebrick", alpha = 1/5) +
  geom_smooth(data = . %>% filter(.width == 0.95),
              aes(ymin = .lower, ymax = .upper),
...
```{r}
nd <- tibble(neocortex.perc_s = 
               seq(from = -2.5, to = 2.5, length.out = 30),
             log_mass_s = 0)

p1 <-
  nd %>% 
  add_epred_draws(b5.7) %>% 
  mean_qi(.epred, .width = c(.5, .95)) %>% 
  
  ggplot(aes(x = neocortex.perc_s, y = .epred)) +
  geom_ribbon(data = . %>% filter(.width == .50),
              aes(ymin = .lower, ymax = .upper),
              fill = "firebrick", alpha = 1/5) +
  geom_smooth(data = . %>% filter(.width == .95),
              stat = "identity",
              aes(ymin = .lower, ymax = .upper),
              color = "firebrick4", fill = "firebrick4",
              alpha = 1/5, linewidth = 1/2) +
...
d <-
  d %>%
  mutate(kcal.per.g_s = (kcal.per.g - mean(kcal.per.g)) /
           sd(kcal.per.g))
```
...
post %>% 
  ...
  theme(axis.text.y = element_text(hjust = 0),
        ...
        panel.grid = element_blank())
```
Let's simulate some made up categories
```{r}
houses <- c("Gryffindor", "Hufflepuff", "Ravenclaw",
            "Slytherin")
...
head(d)
```
sex_c == 0.5 for males and sex_c == 0.5 for females. Intercept is now the average of the mean heights of males and females and slope is the difference when subtracting the mean height of males from the mean height of females.
```{r}
b5.8c <-
  brm(data = d,
      family = gaussian,
